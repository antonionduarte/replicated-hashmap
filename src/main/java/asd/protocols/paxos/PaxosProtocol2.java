...
import asd.paxos.single2.SinglePaxos;
    ...
    private void process(PaxosCmd command) {
        switch (command.getKind()) {
            ...
            case ACCEPT_REQUEST -> {
                ...
                var message = new AcceptRequestMessage(accept.slot(), membership.acceptors,
                        new asd.paxos.single.Proposal(accept.proposal().ballot, accept.proposal().value));
            ...
            case PREPARE_OK -> {
                ...
                var message = new PrepareOkMessage(
                        prepareOk.slot(),
                        prepareOk.ballot(),
                        prepareOk.highestAccept().map(p -> {
                            return new asd.paxos.single.Proposal(p.ballot, p.value);
                        }),
                        false);

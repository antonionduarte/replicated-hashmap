...
import asd.paxos2.single.Paxos;
import asd.paxos2.single.PaxosCmd;
import asd.paxos2.single.PaxosCmdQueue;
import asd.paxos2.single.PaxosConfig;
...
public class PaxosProtocol2 extends GenericProtocol implements Agreement {
    private static class InstanceState {
        ...
        public InstanceState(ProcessId id, PaxosConfig config, Set<ProcessId> membership) {
            ...
            this.paxos = new Paxos(
                    id,
                    queue,
                    config.withMembership(List.copyOf(membership)));
            this.queue = queue;
    ...
    public PaxosProtocol2(Properties props) throws HandlerRegistrationException {
        super(NAME, ID);
            ...
            while (!this.executeQueue.isEmpty()) {
                ...
                switch (cmd.getKind()) {
                    ...
                    case SendAcceptRequest -> {
                        ...
                        var message = new AcceptRequestMessage(
                                instance,
                                state.membershipList,
                                accept.proposal().ballot,
    ...
    private void onAcceptRequest(AcceptRequestMessage msg, Host host, short sourceProto, int channelId) {
        ...
        state.paxos.receiveAcceptRequest(processId, new Proposal(msg.ballot, msg.value));
        this.executeInstanceQueue(instance);
        this.tryPropose();
    }
